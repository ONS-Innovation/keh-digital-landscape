...
const PieChart = ({ 
  ...
  const data = useMemo(() => {
    ...
    if (cloudProvidersOnly && splitSemicolon) {
      ...
      const totalValues = Object.values(categoryCounts).reduce((sum, count) => sum + count, 0) || 1;
    ...
    // Dynamically detect categories if enabled
      ...
      detectedCategories.sort((a, b) => (valueCounts[b] || 0) - (valueCounts[a] || 0));
  ...
  }, [projectsData, categoryField, categories, categoryLabels, getCategoryValue, splitSemicolon, dynamicCategories, maxCategories, cloudProvidersOnly]);
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="custom-tooltip">
          ...
          <p className="count">{`${data.count} ${splitSemicolon ? 'instances' : 'projects'}`} {data.value ? `(${data.value}%)` : ''}</p>
        </div>
      );
    }
    return null;
